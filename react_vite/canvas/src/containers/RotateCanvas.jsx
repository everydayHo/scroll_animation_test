...
import IconFrame from '../assets/icon_AFRAME.png';
...
const RotateCanvas = () => {
	...
	useEffect(() => {
		...
		Events.on(mouseConstraint, 'mousedown', () => {
			const newSelcted = mouseConstraint.body && data[mouseConstraint.body.label];
			newSelcted && setSelected(newSelcted);
		});
		...
		function initImageBoxes() {
			...
			addRect(cw / 2, ch / 2, t1.w, t1.h, { label: 'CSS', champer: { radius: 20 }, render: { sprite: { texture: IconCss, xScale: scale, yScale: scale } } });
			addRect(cw / 2 - t1.w, ch / 2, t1.w, t1.h, { label: 'JS', champer: { radius: 20 }, render: { sprite: { texture: IconJs, xScale: scale, yScale: scale } } });
			addRect(cw / 2 + t1.w, ch / 2, t1.w, t1.h, { label: 'HTML', champer: { radius: 20 }, render: { sprite: { texture: IconHtml, xScale: scale, yScale: scale } } });
			addRect(cw / 2, ch / 2 + t1.h, t1.w, t1.h, { label: 'REACT', champer: { radius: 75 }, render: { sprite: { texture: IconReact, xScale: scale, yScale: scale } } });
			addRect(cw / 2 - t1.w, ch / 2 + t1.h, t1.w, t1.h, { label: 'THREE', champer: { radius: 20 }, render: { sprite: { texture: IconThree, xScale: scale, yScale: scale } } });
			addRect(cw / 2, ch / 2 - t1.h, t2.w, t2.h, { label: 'AFRAME', champer: { radius: 20 }, render: { sprite: { texture: IconFrame, xScale: scale, yScale: scale } } });
		}
		...
		return () => {
			// useEfffect가 언마운트 될 때 matterjs 클리어
